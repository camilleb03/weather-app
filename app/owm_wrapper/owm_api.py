...
class OWM_API:
    def __init__(self):
        # TODO: Regroup params with API_KEY and units and lang
    ...
    def get_5_days_forecast_by_city_name(self, city):
        try:
            res = requests.get(url=self.OWM_BASE_URL+"forecast", params=dict(q=city, units="metrics", APPID=self.OWM_API_KEY))
            res.raise_for_status()
        except requests.exceptions.HTTPError as errh:
            print ("Http Error:",errh)
        except requests.exceptions.ConnectionError as errc:
    ...
    def get_city(city_name):
        try:
            res = requests.get(f"https://nominatim.openstreetmap.org/search", params=dict(city=city_name, format="json", limit=1, addressdetails=1))
            res.raise_for_status()
        except requests.exceptions.HTTPError as errh:
            print ("Http Error:",errh)
        except requests.exceptions.ConnectionError as errc:
